...
// GetEntriesByEnvironment returns all entry paths within an environment
// This method is compatible with the validator.KeePassManager interface
// Note: This implementation doesn't need dbPath, keyfilePath, password as the DB is already open
func (a *DatabaseAdapter) GetEntriesByEnvironment(dbPath, keyfilePath, password, profileName, envName string) ([]string, error) {
	if a.db == nil || a.db.Content == nil {
		return []string{}, nil
	}

	// Navigate to the environment group: profile/HEAD/environment
	envPath := fmt.Sprintf("%s/HEAD/%s", profileName, envName)
	envGroup := a.findGroupByPath(envPath)
	if envGroup == nil {
		// Environment doesn't exist, return empty list (not an error)
		return []string{}, nil
	}

	// Collect all entry paths recursively
	var entryPaths []string
	a.collectEntriesAdapter(envGroup, "", &entryPaths)

	return entryPaths, nil
}

// collectEntriesAdapter recursively collects entry paths from a group and its subgroups
func (a *DatabaseAdapter) collectEntriesAdapter(group *gokeepasslib.Group, currentPath string, result *[]string) {
	// Add entries in current group
	for _, entry := range group.Entries {
		title := a.getEntryField(&entry, "Title")
		if title != "" {
			if currentPath == "" {
				...
				*result = append(*result, fmt.Sprintf("%s/%s", currentPath, title))
			}
		}
	}
	...
	for _, subGroup := range group.Groups {
		newPath := subGroup.Name
		if currentPath != "" {
			newPath = fmt.Sprintf("%s/%s", currentPath, subGroup.Name)
		}
		a.collectEntriesAdapter(&subGroup, newPath, result)
	}
}
